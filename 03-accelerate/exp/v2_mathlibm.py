...
static inline float32x4_t exp4libm(float32x4_t x)
{
    float v[4];
    vst1q_f32(v, x);
...
static inline float32x4_t sin4libm(float32x4_t x)
{
    float v[4];
    vst1q_f32(v, x);
    for (int i = 0; i < 4; ++i) v[i] = sinf(v[i]);
    return vld1q_f32(v);
}
    ...
    for (int qb = 0; qb < q_count; qb += block) {
        ...
        for (int c = 0; c < c_count; ++c) {
            ...
            for (; q + 3 < qe; q += 4) {
                ...
                const float32x4_t g4 = exp4libm(vmulq_n_f32(r4, -s));
                const float32x4_t h4 = sin4libm(vmulq_n_f32(p4, t));
